...
QBCore.Functions.CreateCallback("qb-multicharacter:server:getSkin", function(source, cb, cid)
    local src = source
    QBCore.Functions.ExecuteSql(false, "SELECT * FROM `character_current` WHERE `citizenid` = '"..cid.."'", function(result)
        if result[1] ~= nil then
            cb(result[1].model, result[1].drawables, result[1].props, result[1].drawtextures, result[1].proptextures)
            print(result[1].drawables)
        else
            cb(nil)
        end
    end)
...
function loadHouseData()
	...
	QBCore.Functions.ExecuteSql(false, "SELECT * FROM `houselocations`", function(result)
		...
		TriggerClientEvent("Pt_Garages:client:houseGarageConfig", -1, HouseGarages)
		TriggerClientEvent("Pt_Houses:client:setHouseConfig", -1, Houses)
	end)
end